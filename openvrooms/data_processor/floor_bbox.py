...
class FloorBBox(object):
    ...
    def generate_v_bbox(self, z_min, z_max):
        if self.reader == None:
        ...
        v_bbox = np.vstack([
            np.hstack([bounds, np.ones((4, 1))*z_max]),
            np.hstack([bounds, np.ones((4, 1))*z_min])
        ]) # (8, 3)
    ...
    def generate_floor(self, save_path, floor_thickness=0.2):
        parent_dir = Path(save_path).parent
        assert os.path.isdir(parent_dir), f"File {parent_dir} doesn't exist!"
        floor_thickness = max(floor_thickness, 0.)
        v_bbox = self.generate_v_bbox(z_min=-floor_thickness, z_max=0.)
...
if __name__ == '__main__':
    obj_file_name = 'scene0420_01_floor.obj'
    save_path = 'floor.obj'

    floor = FloorBBox()
    floor.parse_obj(obj_file_name)
    floor.generate_floor(save_path, floor_thickness=0.2)
